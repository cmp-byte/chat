...
public class Message2Complex extends Message2 {
    ...
    public boolean get_file(String directory){
        if(attachment!=null){
            CloseableHttpClient httpclient = HttpClients.createDefault();
            HttpGet httpGet = new HttpGet(fileServerURL+"api/files/"+attachment);
            try (CloseableHttpResponse response = httpclient.execute(httpGet)) {
                HttpEntity responseEntity = response.getEntity();
                byte[] array = EntityUtils.toByteArray(responseEntity);
                try(FileOutputStream stream = new FileOutputStream(directory+"/"+attachment)){
                    stream.write(array);
                }
                EntityUtils.consume(responseEntity);
                return true;
